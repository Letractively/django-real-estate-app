...
(function($) {
  ...
  $.extend($.facebox, {
    ...
    <div class="facebox" style="display:none;"> \
      <div class="popup"> \
        ...
        <a href="#" class="close"><img src="/media-real/img/closelabel.png" title="close" class="close_image" /></a> \
      </div> \
    </div>'
    },
  ...
  function init(settings) {
    ...
    $('#'+$.facebox.settings.id+' .close').click(function(e){
		...
		if(settings.type_field != 'input' && ($.facebox.settings.ajax_url && $.facebox.settings.id != false || $.facebox.settings.ajax_url && $.facebox.settings.id != undefined) ) ajaxRefreshField(settings.ajax_url,$.facebox.settings.id,settings);
	})
  ...
  function getLabel(labels, item) {
    label='';
    for (field in labels) {
      label+=' '+item.fields[labels[field]]
    }
    return label
  }

  // ajaxRefreshField(url, id) is a refresh a id object
  function ajaxRefreshField(url,id,settings) {
		$.ajax({
		          type: "GET",
	            url: url,
	            dataType: "json",
	          ...
	          success: function(retorno){
                // TODO: fazer para mais tipos de widgets
                labels=settings.field.split(',');
                label=''

                if (settings.type_field == 'select') {
      		        ...
      		        $.each(retorno, function(i, item){
                                ...
                                label = getLabel(labels,item)
      							...
      							if (item.pk == selected) {
      			                    $("select#"+id).append('<option selected="selected" value="' + item.pk + '">' + label + '</option>');
      							} else {
      			                    $("select#"+id).append('<option value="' + item.pk + '">' + label + '</option>');
      							}
      						});
      					}
