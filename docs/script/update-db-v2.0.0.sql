insert into real_estate_app_apps_propertys_position_of_sun select id, position, logical_exclude from real_estate_app_positionofsun;
insert into real_estate_app_apps_propertys_aditionalthings select id, name, logical_exclude from real_estate_app_aditionalthings;
...
insert into real_estate_app_apps_propertys_classification select id, classification, logical_exclude from real_estate_app_classification; 
insert into real_estate_app_apps_propertys_district select id,district, logical_exclude, state from real_estate_app_district;
insert into real_estate_app_apps_propertys_status_property SELECT id, statusproperty, logical_exclude FROM real_estate_app_statusproperty; 
insert into real_estate_app_apps_propertys_property (id, address, slug, zip_code, price, district_fk_id, condominio, iptu, classification_fk_id, statusproperty_fk_id, state,rooms, baths, garage, elevator,  furnishing, featured,  under_contruction, private_area,  position_of_sun_id,date_init, date_end, description,enable_publish,gmap_point_x,gmap_point_y, code_property,create_date, logical_exclude ) select id, address, slug, zip_code, price, district_fk_id, condominio, iptu, classification_fk_id, statusproperty_fk_id, state,rooms, baths, garage, elevator,  furnishing, featured,  under_contruction, private_area,  position_of_sun_id,date_init, date_end, description,enable_publish,gmap_point_x,gmap_point_y, code_property, date(now()), false from real_estate_app_property; 
...
SELECT setval('real_estate_app_apps_real_estate_files_files_id_seq',(select max(id)+1 from real_estate_app_files));
